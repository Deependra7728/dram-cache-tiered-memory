...
re-verified against the actual source this session: both papers (re-read
...
| Admission ordering | conflict → WB-pressure → ORB-full → admit (`recvTimingReq`) | Identical order in `add_rq`/`add_wq` | **Equivalent** | None | Direct port, re-verified this session at `policy_manager.cc:296-362` |
...
| Timing requirement | Link latency exact per the paper's 3 values | Verified exact this session | **Ready** |
...
| **Sum** | | **62 tests, 8 files** | **62/62 PASS** (re-confirmed this session before the audit) |
