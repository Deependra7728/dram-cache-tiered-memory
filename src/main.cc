...
// ----------------------------------------------------------------------------
// DRAM Cache Manager experiment knobs (docs/case_study_reproduction_plan.md).
// Added so the paper's Case Study 1/2/3 configurations can be selected
// entirely via the command line -- no source-code edits or recompiles
// between runs. Defaults match every prior stage's behavior exactly
// (DCM_POLICY_BASELINE_CASCADE_LAKE, bypass off, 0 link latency), so
// omitting these flags reproduces the exact pre-existing binary.
// ----------------------------------------------------------------------------
static std::string knob_dcm_policy = "baseline"; // "baseline" | "bear" | "oracle"
static uint8_t knob_dcm_bypass = 0;               // Case Study 1's "No DRAM Cache" configuration
static uint64_t knob_dcm_link_latency_ns = 0;     // Case Study 3: 0 (off) | 100 | 500 | 1000
...
// Prints every DRAM_CACHE_MANAGER::stats field relevant to the paper's
// Case Study metrics (docs/case_study_reproduction_plan.md), clearly
// labeled by name so results from different --dcm_policy/--dcm_bypass/
// --dcm_link_latency_ns runs can be told apart and compared directly.
// This is a NEW print (DCM_STATS was never surfaced in champsim's
// output before this stage) -- purely additive, changes nothing about
// any existing statistic.
void print_dcm_stats()
{
    DCM_STATS &s = uncore.DCM.stats;
    cout << endl;
    cout << "DRAM Cache Manager Statistics (policy=" << knob_dcm_policy
         << (knob_dcm_bypass ? ", BYPASSED" : "") << ", link_latency_ns=" << knob_dcm_link_latency_ns
         << ")" << endl;

    if (knob_dcm_bypass) {
        // Bypass mode never touches ORB/CRB/WB/tag-metadata (see
        // docs/bypass_mode.md) -- only these three counters are
        // meaningful; printing the normal-mode fields below would show
        // all zeros and could be misread as "the DRAM cache did
        // nothing" rather than "the DRAM cache was not in the path at
        // all," so they are reported separately and explicitly.
        cout << " DCM BYPASS_READS: " << setw(10) << s.bypassReads
             << "  BYPASS_WRITES: " << setw(10) << s.bypassWrites
             << "  BYPASS_COMPLETED_READS: " << setw(10) << s.bypassCompletedReads << endl;
        return;
    }

    cout << " DCM TOTAL_REQUESTS: " << setw(10) << s.totalRequests << "  READS: " << setw(10) << s.readRequests
         << "  WRITES: " << setw(10) << s.writeRequests << endl;
    cout << " DCM HITS: " << setw(10) << s.numTotHits << "  MISSES: " << setw(10) << s.numTotMisses
         << "  COLD_MISSES: " << setw(10) << s.numColdMisses << "  HOT_MISSES: " << setw(10) << s.numHotMisses
         << endl;
    cout << " DCM RD_HIT: " << setw(10) << s.numRdHit << "  WR_HIT: " << setw(10) << s.numWrHit
         << "  RD_MISS_CLEAN: " << setw(10) << s.numRdMissClean << "  RD_MISS_DIRTY: " << setw(10)
         << s.numRdMissDirty << "  WR_MISS_CLEAN: " << setw(10) << s.numWrMissClean << "  WR_MISS_DIRTY: "
         << setw(10) << s.numWrMissDirty << endl;
    cout << " DCM LOCAL_READS: " << setw(10) << s.localReads << "  LOCAL_WRITES: " << setw(10) << s.localWrites
         << "  FAR_READS: " << setw(10) << s.farReads << "  FAR_WRITES: " << setw(10) << s.farWrites
         << "  CACHE_FILLS: " << setw(10) << s.cacheFills << endl;
    cout << " DCM WB_INSERTIONS: " << setw(10) << s.wbInsertions << "  WB_DRAINS: " << setw(10) << s.wbDrains
         << "  WB_FULL_REJECTS: " << setw(10) << s.wbFullRejects << "  WB_DISPATCH_RETRIES: " << setw(10)
         << s.wbDispatchRetries << endl;
    cout << " DCM NEAR_DISPATCH_RETRIES: " << setw(10) << s.nearDispatchRetries << "  FAR_DISPATCH_RETRIES: "
         << setw(10) << s.farDispatchRetries << endl;
    cout << " DCM CONFLICTS(CRB_INSERTS): " << setw(10) << s.crbInserts << "  CRB_PROMOTIONS: " << setw(10)
         << s.crbPromotions << "  CRB_FULL_REJECTS: " << setw(10) << s.crbFullRejects << "  ORB_FULL_REJECTS: "
         << setw(10) << s.orbFullRejects << endl;
    cout << " DCM ACCESS_AMPLIFICATION: " << s.accessAmplification() << "  (avg local+far sub-ops per request)"
         << endl;
    cout << " DCM ORB_OCCUPANCY avg: " << s.avgOrbOccupancy() << "  max: " << setw(10) << s.orbMaxOccupancy
         << endl;
    cout << " DCM CRB_OCCUPANCY avg: " << s.avgCrbOccupancy() << "  max: " << setw(10) << s.crbMaxOccupancy
         << endl;
    cout << " DCM WB_OCCUPANCY  avg: " << s.avgWbOccupancy() << "  max: " << setw(10) << s.wbMaxOccupancy
         << endl;

    // Bandwidth utilization (paper Figure 9's metric): fraction of each
    // controller's theoretical peak bandwidth actually used, computed
    // from the DCM's own local/far operation counts (each moving
    // BLOCK_SIZE bytes) over the elapsed ROI cycles, against each
    // controller's configured peak (DRAM_CHANNEL_WIDTH * DRAM_MTPS
    // bytes/microsecond, converted to bytes/cycle via CPU_FREQ -- see
    // inc/dram_cache_manager.h's near/far configuration comment for the
    // same derivation used to set DRAM_MTPS in the first place).
    // NUM_CPUS==1 in this build, so ooo_cpu[0]'s elapsed cycles are used
    // directly; this would need a per-CPU/shared-channel treatment if
    // NUM_CPUS were ever changed (documented gap, see limitations.md).
    // ooo_cpu[].finish_sim_cycle is ALREADY the ROI duration -- it is
    // assigned as (current_core_cycle - begin_sim_cycle) at simulation
    // completion. Subtracting begin_sim_cycle a second time (as this
    // code previously did) double-counts the warmup and UNSIGNED-
    // UNDERFLOWS whenever the ROI is shorter in cycles than the warmup,
    // silently reporting ~1e-16% instead of failing
    // (docs/final_independent_audit.md, HIGH-1).
    uint64_t elapsedCycles = ooo_cpu[0].finish_sim_cycle;
    if (elapsedCycles > 0) {
        double localUtilization = dcmBandwidthUtilization(
            s.localReads + s.localWrites, uncore.DRAM_CACHE_DEVICE.DRAM_MTPS, elapsedCycles);
        double farUtilization =
            dcmBandwidthUtilization(s.farReads + s.farWrites, uncore.DRAM.DRAM_MTPS, elapsedCycles);
        cout << " DCM LOCAL_BW_UTILIZATION: " << (100.0 * localUtilization) << "%  FAR_BW_UTILIZATION: "
             << (100.0 * farUtilization) << "%  (over " << elapsedCycles << " ROI cycles)" << endl;
    } else {
        cout << " DCM LOCAL_BW_UTILIZATION: -  FAR_BW_UTILIZATION: -  (no ROI cycles recorded)" << endl;
    }

    // Run-validity invariants (docs/final_independent_audit.md
    // MEDIUM-1): a non-empty ORB at end of simulation means at least one
    // request never completed. Printed unconditionally so a result file
    // is self-validating.
    cout << " DCM WARMUP_TAG_UPDATES: " << setw(10) << s.warmupTagUpdates
         << "  DISPATCH_MERGE_RETRIES: " << setw(10) << s.dispatchMergeRetries << endl;
    cout << " DCM COMPLETED_READS_TO_LLC: " << setw(10) << s.completedReadsToLLC
         << "  COMPLETED_WRITES: " << setw(10) << s.completedWrites
         << "  COMPLETED_FROM_NEAR: " << setw(10) << s.completedFromNear
         << "  COMPLETED_FROM_FAR: " << setw(10) << s.completedFromFar << endl;
    cout << " DCM RESIDUAL ORB: " << uncore.DCM.ORB.size() << "  CRB: " << uncore.DCM.CRB.size()
         << "  WB: " << uncore.DCM.WB.size()
         << "  pendingNear: " << uncore.DCM.pendingNearDispatches.size()
         << "  pendingFar: " << uncore.DCM.pendingFarDispatches.size()
         << "  pendingResp: " << uncore.DCM.pendingResponses.size();

    // ChampSim stops the instant the simulation-instruction count is
    // reached; it does NOT drain the memory system first. A SMALL
    // residual is therefore normal -- those requests were simply in
    // flight at the cutoff, and they reconcile exactly against the
    // completion counters above. What is NOT normal is a JAMMED
    // manager: the deadlock signature from
    // docs/final_independent_audit.md is a stranded ORB entry that
    // permanently owns its DRAM-cache index, which drives the ORB to
    // capacity and/or backs the CRB up until it starts rejecting.
    // Distinguish the two rather than warning on every healthy run,
    // which would only train the reader to ignore the line.
    bool jammed = (uncore.DCM.ORB.size() >= DCM_ORB_MAX_SIZE) ||
                  (uncore.DCM.CRB.size() >= DCM_CRB_MAX_SIZE) || (s.crbFullRejects > 0) ||
                  (s.orbFullRejects > 0);
    if (uncore.DCM.ORB.empty() && uncore.DCM.CRB.empty() && uncore.DCM.WB.empty())
        cout << "   [OK: fully drained]" << endl;
    else if (!jammed)
        cout << "   [OK: in flight at cutoff -- ChampSim stops without draining]" << endl;
    else
        cout << "   [*** WARNING: manager appears jammed -- run may be invalid ***]" << endl;
}
...
void finish_warmup()
{
    ...
    // Clear the DRAM cache manager's ROI statistics at the same
    // warmup->ROI boundary the caches use, so DCM numbers are directly
    // comparable to the LLC numbers printed beside them. This does NOT
    // clear the tag/metadata store -- the DRAM cache stays warmed, which
    // is the whole point of warming it (docs/final_independent_audit.md,
    // HIGH-2). Called once, not per-CPU: the DCM is a single shared
    // object, unlike the per-core caches above.
    uncore.DCM.resetROIStats();
...
int main(int argc, char** argv)
{
    ...
    while (1) {
        static struct option long_options[] =
        {
            ...
            // DRAM Cache Manager experiment knobs (docs/case_study_reproduction_plan.md).
            // Given distinct integer codes (not letters) since getopt_long_only's
            // short-option string only covers the pre-existing single-letter
            // flags above -- these are long-form-only.
            {"dcm_policy", required_argument, 0, 1001},
            {"dcm_bypass", no_argument, 0, 1002},
            {"dcm_link_latency_ns", required_argument, 0, 1003},
            {0, 0, 0, 0}
        };
        ...
        switch(c) {
            ...
            case 't':
                ...
                break;
            case 1001:
                knob_dcm_policy = std::string(optarg);
                break;
            case 1002:
                knob_dcm_bypass = 1;
                break;
            case 1003:
                knob_dcm_link_latency_ns = atol(optarg);
                break;
            default:
                abort();
        }
    ...
    // DRAM timing is a per-MEMORY_CONTROLLER-instance setting (see
    // docs/gem5_to_champsim_mapping.md). uncore.DRAM_CACHE_DEVICE (near)
    // and uncore.DRAM (far) are now configured with the paper's own
    // declared technology profiles -- HBM2 near (32 GB/s), DDR4 far
    // (19.2 GB/s), Table I -- via configureNearAsHBM2()/configureFarAsDDR4()
    // (inc/dram_cache_manager.h). See that header and
    // docs/gem5_to_champsim_mapping.md for the exact
    // paper-value -> ChampSim-value derivation and the documented
    // latency-fidelity gap (tRP/tRCD/tCAS not differentiated).
    configureNearAsHBM2(uncore.DRAM_CACHE_DEVICE);
    configureFarAsDDR4(uncore.DRAM);

    // Preserve the pre-existing --low_bandwidth knob's effect (quarter
    // the transfer rate) on top of the paper-derived base configuration,
    // applied to both controllers, so this ChampSim knob keeps working
    // exactly as it did before near/far were split into two controllers.
    if (knob_low_bandwidth) {
        uncore.DRAM_CACHE_DEVICE.DRAM_MTPS /= 4;
        uncore.DRAM_CACHE_DEVICE.DRAM_DBUS_RETURN_TIME = computeDbusReturnTime(uncore.DRAM_CACHE_DEVICE.DRAM_MTPS);
        uncore.DRAM.DRAM_MTPS /= 4;
        uncore.DRAM.DRAM_DBUS_RETURN_TIME = computeDbusReturnTime(uncore.DRAM.DRAM_MTPS);
    }

    // Apply the DCM experiment knobs (docs/case_study_reproduction_plan.md):
    // policy selection (Case Study 2), bypass mode (Case Study 1's
    // "No DRAM Cache" configuration), and far-link latency (Case Study 3).
    // Defaults (baseline, bypass off, 0ns) reproduce every prior stage's
    // exact behavior when no flag is given -- this block is additive, not
    // a behavior change to the pre-existing binary.
    if (knob_dcm_policy == "baseline") {
        uncore.DCM.policy = DCM_POLICY_BASELINE_CASCADE_LAKE;
    } else if (knob_dcm_policy == "bear") {
        uncore.DCM.policy = DCM_POLICY_BEAR_WR_OPT;
    } else if (knob_dcm_policy == "oracle") {
        uncore.DCM.policy = DCM_POLICY_ORACLE;
    } else {
        cout << "Unrecognized --dcm_policy value \"" << knob_dcm_policy
             << "\" (expected baseline|bear|oracle) -- exiting." << endl;
        return 1;
    }
    if (knob_dcm_bypass)
        uncore.DCM.setBypassDcache(true);
    if (knob_dcm_link_latency_ns > 0)
        uncore.DCM.setLinkLatency((uint64_t)((knob_dcm_link_latency_ns * CPU_FREQ) / 1000));

    cout << "DRAM Cache Manager policy: " << knob_dcm_policy
         << (knob_dcm_bypass ? " (BYPASSED -- No-DRAM-Cache mode)" : "")
         << " link_latency_ns: " << knob_dcm_link_latency_ns << endl;

    // NOTE: DRAM_SIZE is ChampSim's single global off-chip-memory-array
    // sizing macro (bank/row/column counts) -- it does not represent the
    // paper's 128 MB DRAM-cache capacity (that's DCM_DRAM_CACHE_SIZE,
    // used only for tag/index math in DRAM_CACHE_MANAGER, not for sizing
    // this timing-only MEMORY_CONTROLLER model). Printed here only for
    // the channel/width/rate fields, matching the pre-existing message's
    // intent; not a claim that near/far have different backing capacities.
    printf("DRAM Cache (near, HBM2-like) Channels: %u Width: %u-bit Data Rate: %u MT/s\n",
            DRAM_CHANNELS, 8*DRAM_CHANNEL_WIDTH, uncore.DRAM_CACHE_DEVICE.DRAM_MTPS);
    printf("Off-chip DRAM (far, DDR4-like) Size: %u MB Channels: %u Width: %u-bit Data Rate: %u MT/s\n",
            DRAM_SIZE, DRAM_CHANNELS, 8*DRAM_CHANNEL_WIDTH, uncore.DRAM.DRAM_MTPS);
	...
	ooo_cpu[i].L1I.l1i_prefetcher_cache_fill = cpu_l1i_prefetcher_cache_fill;
        ...
        // LLC's lower_level is now the DRAM Cache Manager instead of the
        // DRAM controller directly. DCM forwards to DRAM_CACHE_DEVICE
        // (near) and DRAM (far) internally -- see inc/uncore.h and
        // docs/gem5_to_champsim_mapping.md. DRAM_CACHE_DEVICE/DRAM's own
        // upper_level_icache/dcache pointers are wired to &uncore.DCM
        // inside DRAM_CACHE_MANAGER's constructor (not here), the same way
        // this loop used to wire uncore.DRAM's pointers to &uncore.LLC.
        uncore.LLC.lower_level = &uncore.DCM;

        // DRAM CACHE MANAGER
        uncore.DCM.fill_level = FILL_DRAM;
        uncore.DCM.upper_level_icache[i] = &uncore.LLC;
        uncore.DCM.upper_level_dcache[i] = &uncore.LLC;
    ...
    while (run_simulation) {
        ...
        uncore.DCM.operate(); // operates DRAM_CACHE_DEVICE (near) and DRAM (far) internally
        uncore.LLC.operate();
    }
